...
class TwitchWebhook extends EventEmitter {
  ...
  listen(...args) {
    ...
    return new Promise((resolve) => {
      if (!args.length) {
        this._server.listen(this._options.listen.port, this._options.listen.host,
          () => resolve());
      } else {
  ...
  _request(mode, topic, options) {
    ...
    if (this._options.secret) {
      let secret = crypto.createHmac('sha256', this._options.secret)
        .update(topic).digest('hex');
  ...
  _processConnection(request, response) {
    ...
    switch (queries['hub.mode']) {
      case 'denied':
        ...
        break;
      case 'unsubscribe':
        delete this._secrets[queries['hub.topic']];
      case 'subscribe':
        response.writeHead(200, { 'Content-Type': 'text/plain' });
  ...
  _processUpdates(request, response) {
    ...
    request.on('end', () => {
      ...
      const topicName = topic && url.parse(topic).pathname.replace('/helix/', '');
      if (!topic || !topicName) {
        response.writeHead(400, { 'Content-Type': 'text/plain' });
        response.end();
        return;
      ...
      if (this._options.secret) {
        const storedSign = crypto.createHmac('sha256', this._secrets[topic])
